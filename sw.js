const CACHE_NAME = 'phelox-pwa-2026-09-18-v16';
const APP_SHELL = ['/index.html?v=cloud-v16', '/manifest.webmanifest', '/phelox-logo-192-v2.png', '/phelox-logo-512-v2.png', '/apple-touch-icon-v2.png'];
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request, {cache: 'no-store'}).then(response => {
      const copy = response.clone(); caches.open(CACHE_NAME).then(cache => cache.put('/index.html?v=cloud-v16', copy)); return response;
    }).catch(() => caches.match('/index.html?v=cloud-v16')));
    return;
  }
